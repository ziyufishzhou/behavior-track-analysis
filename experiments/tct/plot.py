...
# ==================== 2. 全局参数 ====================
FILE_PATH = str(TCT_SUMMARY / "TCT_Complete_Data_0309_1439.xlsx")  # TODO: auto-find latest
OUTPUT_DIR = str(TCT_FIGURES)

if not os.path.exists(OUTPUT_DIR):
    os.makedirs(OUTPUT_DIR)

# --- 颜色方案更新 ---
C_TIME_TOY = '#48C9B0'    # 青色 (Medium Aquamarine)
C_TIME_LIVE = '#2E86C1'   # 蓝色 (Strong Blue)

C_PI_GRAY = '#D5D8DC'     # 浅灰色 (Light Gray)
C_PI_BLUE = '#5DADE2'     # 浅蓝色 (Sky Blue - 与Time图蓝色呼应但略浅)
...
def plot_tct_new_palette(df, phase):
    ...
    for i, grp_name in enumerate(groups):
        ...
        ax1.annotate('', xy=(0.95, GROUP_TITLE_Y), xytext=(1.8, GROUP_TITLE_Y), xycoords='axes fraction', arrowprops=dict(arrowstyle='-', lw=0.6))

    plt.savefig(os.path.join(OUTPUT_DIR, f"TCT_Palette_Final_{phase}.pdf"))
    plt.show()

if __name__ == "__main__":
    raw_df = pd.read_excel(FILE_PATH)
    for p in ['S', 'N']: plot_tct_new_palette(raw_df, p)